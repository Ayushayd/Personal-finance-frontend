...
const API = axios.create({
  baseURL: "https://finance-system1-production.up.railway.app/api",
});
...
export const setExpenseLimit = (limit) =>
  API.post("/user/limit", { monthlyLimit: limit });
export const updateIncome = (id, income) =>
  API.put(`/user/income/${id}`, income);
...
export const getExpensesByCategoryForAdmin = () =>
  API.get(`/admin/expenses/expenses-by-category`);
export const getSystemWideReports = () => API.get("/admin/report");
export const setExpenseLimitForUser = (userId, limit) =>
  API.post(`/admin/limit/${userId}`, { monthlyLimit: limit });
